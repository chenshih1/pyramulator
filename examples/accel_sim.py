...
class Accelerator:
    def __init__(self, num_lanes=4, vector_size=4096, elem_size=8,
                 tile_size=64, max_outstanding=32, compute_cycles=4,
                 base_addr_a=0x0_0000, base_addr_b=0x10_0000,
    ...
    def setup(self, dram_config: Config):
        self.mem = MemorySystem(dram_config, cacheline=64,
                                num_cores=self.num_lanes)
        ...
        for i in range(count):
            ...
            while True:
                if self.outstanding + 2 > self.max_outstanding:
                    ...
                    self.stall_cycles += 1
                    continue
                ok_a = self.mem.send_read(self.base_addr_a + byte_off,
                                          core_id=lane, callback=on_read_done)
                ok_b = self.mem.send_read(self.base_addr_b + byte_off,
                                          core_id=lane, callback=on_read_done)
                if ok_a and ok_b:
                    self.outstanding += 2
                    break
        ...
        for i in range(count):
            ...
            while True:
                if self.outstanding >= self.max_outstanding:
                    ...
                    self.stall_cycles += 1
                    continue
                if self.mem.send_write(self.base_addr_c + byte_off,
                                       core_id=lane, callback=on_write_done):
                    self.outstanding += 1
                    break
                self.mem.tick()
                self.clk += 1
    ...
    def report(self):
        ...
        avg_read_lat = (sum(self.read_latencies) / total_reads
                        if total_reads else 0)
        avg_write_lat = (sum(self.write_latencies) / total_writes
                         if total_writes else 0)

        logger.info("=" * 60)
        logger.info("%s", f"{'Accelerator Simulation Report':^60}")
        logger.info("=" * 60)
        logger.info("  Lanes:           %d", self.num_lanes)
        logger.info("  Vector:          %d x %dB", self.vector_size,
                    self.elem_size)
        ...
        logger.info("  Sim time:        %.0f ns (%.4f ms)", time_ns,
                    time_ns / 1e6)
        logger.info("  Reads:           %d", total_reads)
        logger.info("  Writes:          %d", total_writes)
        logger.info("  Stall cycles:    %d", self.stall_cycles)
        logger.info("  Avg read lat:    %.1f cyc (%.1f ns)", avg_read_lat,
                    avg_read_lat * tck)
        logger.info("  Avg write lat:   %.1f cyc (%.1f ns)", avg_write_lat,
                    avg_write_lat * tck)
