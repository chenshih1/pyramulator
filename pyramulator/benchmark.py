...
def benchmark_latency(config: Config | dict[str, Any], num_requests: int = 256,
                      cacheline: int = 64, mode: str = "sequential",
                      seed: int | None = None,
                      max_addr: int = 1 << 26) -> dict[str, float | int]:
    ...
    latencies = []
    for addr in addresses(mode, num_requests, cacheline,
                          seed=seed, max_addr=max_addr):
        while not mem.send_read(addr, callback=lambda info: latencies.append(
                info.depart - info.arrive)):
            mem.tick()
    mem.run_until_idle()
    if not latencies:
        return {"avg": 0.0, "min": 0, "max": 0, "tck": mem.tck,
                "completed": 0}
    return {"avg": sum(latencies) / len(latencies),
            "min": min(latencies), "max": max(latencies),
            "tck": mem.tck, "completed": len(latencies)}


def benchmark_bandwidth(config: Config | dict[str, Any],
                        num_requests: int = 256, cacheline: int = 64,
                        queue_depth: int = 32,
                        max_cycles: int = 200_000) -> dict[str, float | int]:
    ...
    if first_clk[0] is None or completed[0] < 2:
        return {"bandwidth_gbs": 0.0, "completed": completed[0],
                "cycles": 0, "tck": mem.tck}
    ...
    return {"bandwidth_gbs": bandwidth, "completed": completed[0],
            "cycles": active_cycles, "tck": mem.tck}


def benchmark_all(config: Config | dict[str, Any], num_requests: int = 256,
                  cacheline: int = 64) -> dict[str, dict[str, float | int]]:
